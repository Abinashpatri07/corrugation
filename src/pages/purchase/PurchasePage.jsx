...
const PurchasePage = () => {
  ...
  // Fetch Purchase Orders when page opens
  useEffect(() => {
    fetchPurchaseOrders();
  }, []);

  // ======================================================
  // Fetch Bills when Bills tab is selected
  // ======================================================
  useEffect(() => {
    if (activeTab === 'Bills') {
      fetchBills();
  ...
  // Get Purchase Orders from backend
  ...
  const fetchBills = async () => {
    try {
      ...
      // If API fails, keep the table empty
      setBills([]);
    } finally {
      setLoading(false);
    }
              ...
              {activeTab === 'Expenses' ? (
                expenses.map((expense, idx) => (
                  ...
                  >
                    ...
                    <td className="py-2.5 px-2 text-[#1a233a] font-medium text-left whitespace-nowrap">{expense.reference || ''}</td>
                    <td className="py-2.5 px-2 text-[#1a233a] font-medium text-left whitespace-nowrap">{expense.vendor_name}</td>
                    <td className="py-2.5 px-2 text-[#1a233a] font-medium text-left whitespace-nowrap">{expense.paidThrough}</td>
                    <td className="py-2.5 px-2 text-[#1a233a] font-medium text-left whitespace-nowrap">{expense.customerName}</td>
                    <td className="py-2.5 px-2 text-[#1a233a] font-medium text-left whitespace-nowrap">{expense.status}</td>
                    <td className="py-2.5 pr-3 text-[#1a233a] font-medium whitespace-nowrap text-left">{expense.amount}</td>
                ...
                bills.map((bill) => (
                  ...
                  <tr
                    ...
                    // Open the selected bill using its real database ID
                    onClick={() =>
                      navigate(`/purchase/bill/${bill.bill_id}`)
                    }

                    className={`border-b border-gray-100 hover:bg-gray-50/50 transition-colors text-[13px] cursor-pointer
        ${selectedRows.includes(bill.bill_id)
                        ? 'bg-blue-50'
                        : 'bg-white'
                      }`}
                  >
                    ...
                    {/* Checkbox */}
                    <td
                      className="py-2.5 pl-4 pr-3 text-center"
                      onClick={(e) => e.stopPropagation()}
                    >
                      <input
                        type="checkbox"

                        checked={selectedRows.includes(bill.bill_id)}

                        onChange={() => toggleRow(bill.bill_id)}
                    ...
                    {/* ==================================================
                          Bill Date
                      ================================================== */}
                    <td className="py-2.5 px-2 text-[#1a233a] font-medium whitespace-nowrap">
                    ...
                    <td className="py-2.5 px-2 whitespace-nowrap">
                      ...
                      <span className="text-blue-500 hover:underline cursor-pointer font-medium text-left">
                        ...
                        {bill.bill_number || bill.bill_code || ''}
                    ...
                    {/* ==================================================
                          Vendor Name
                      ================================================== */}
                    <td className="py-2.5 px-2 text-[#1a233a] font-medium uppercase whitespace-nowrap">
                    ...
                    {/* ==================================================
                          Total Amount
                      ================================================== */}
                    <td className="py-2.5 px-2 text-[#1a233a] font-medium text-left whitespace-nowrap">
                    ...
                    {/* ==================================================
                        Due Date
                    ================================================== */}
                    <td className="py-2.5 px-2 text-[#1a233a] font-medium whitespace-nowrap text-left">
                ...
                procurements.map((pr, idx) => (
                  ...
                  >
                    ...
                    <td className="py-2.5 px-2 text-[#1a233a] font-medium whitespace-nowrap">{pr.vendor_name}</td>
                    <td className="py-2.5 px-2 text-[#1a233a] font-medium whitespace-nowrap">{pr.material}</td>
                    <td className="py-2.5 px-2 text-[#1a233a] font-medium whitespace-nowrap text-left">{pr.quantity}</td>
                    <td className="py-2.5 pr-3 text-[#1a233a] font-medium text-left">{pr.amount}</td>
                  </tr>
                ...
                orders.map((order) => (
                  <tr
                    ...
                    onClick={() =>
                      navigate(`/purchase/order/${order.purchase_order_id}`)
                    }
                    className={`border-b border-gray-100 hover:bg-gray-50/50 transition-colors text-[13px] cursor-pointer
                        ${selectedRows.includes(order.purchase_order_id) ? 'bg-blue-50' : 'bg-white'}
                   `}
                  >
                    ...
                    <td className="py-2.5 pl-4 pr-3 text-center" onClick={e => e.stopPropagation()}>
                      <input
                        type="checkbox"
                        checked={selectedRows.includes(order.purchase_order_id)}
                        onChange={() => toggleRow(order.purchase_order_id)}
                        className="rounded border-gray-300 text-blue-600 focus:ring-blue-500 w-3.5 h-3.5 cursor-pointer"
                      />
                    </td>
                    {/* Date */}
                    <td className="py-2.5 px-2 text-[#1a233a] font-medium whitespace-nowrap">{order.po_date
                    ...
                    {/* Reference */}
                    <td className="py-2.5 px-2 text-[#1a233a] font-medium text-left whitespace-nowrap">{order.pr_number || ''}</td>
                    {/* Vendor */}
                    <td className="py-2.5 px-2 text-[#1a233a] font-medium text-left whitespace-nowrap">{order.vendor_name || order.vendor_code || 'Unknown Vendor'}</td>
                    {/* Status */}
                    <td className="py-2.5 px-2 text-left">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-[11px] font-medium ${statusColors[order.delivery_status] || 'bg-gray-100 text-gray-600'}`}>
                        {order.delivery_status}
                      </span>
                    </td>
                    {/* Recieved */}
                    <td className="py-2.5 px-2 text-[#1a233a] font-medium text-left whitespace-nowrap">---</td>
                    {/* Billed */}
                    <td className="py-2.5 px-2 text-[#1a233a] font-medium text-left whitespace-nowrap">---</td>
                    {/* Amount */}
                    <td className="py-2.5 pr-3 text-[#1a233a] font-medium whitespace-nowrap text-left">₹{Number(order.estimated_total || 0).toLocaleString('en-IN', {
                      minimumFractionDigits: 2
                    })}</td>
                  </tr>
                ))
          ...
          {/* Empty state if no orders */}
          {activeTab === 'Bills' && bills.length === 0 && !loading && (
            <div className="flex flex-col items-center justify-center py-16 text-gray-400">
          ...
          {activeTab === 'Purchase Order' && orders.length === 0 && !loading && (
            <div className="flex flex-col items-center justify-center py-16 text-gray-400">
              <p className="text-[14px]">
